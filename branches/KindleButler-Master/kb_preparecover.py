...
def draw(imgin, title, seqnum, position):
    if title==None:
        print ("Drawing:", seqnum, "No title")
    imgname = os.path.splitext(imgin)[0]
    imgext = os.path.splitext(imgin)[1]
    imgout = imgname + '_stamped' + imgext
    shutil.copy(imgin,imgout)
    resize(imgout)
    txt2img(title, seqnum, imgout, imgout,position)

def resize(img_in):
    try:
        from PIL import Image
        img = Image.open(img_in)
        img = img.resize((217,330), Image.ANTIALIAS)
...
def main(argv=sys.argv):
    ...
    draw(args.input_file, args.title, args.sequence_number,args.position)
